...
{
    [_scrollView setContentSize:CGSizeMake( SCREEN_WIDTH, _scrollView.frame.size.height+1300)];
    _scrollView.bounces = NO;
...
-(void)GetListBanks
{
    ...
    [Controller GetListBanksByCountryID:[NSString stringWithFormat:@"%@", selectCountryCode] withSuccess:^(id responseObject)
     {
         [HUD removeFromSuperview];
     }andFailure:^(NSString *String){
         
         NSLog(@"Bank String..%@", String);
         NSError *error;
         NSData *jsonData = [String dataUsingEncoding:NSUTF8StringEncoding];
         NSDictionary *json = [NSJSONSerialization JSONObjectWithData:jsonData
                                                              options:NSJSONReadingMutableContainers error:&error];
         ...
         if ( Status  == YES)
         {
             bankArray= [ payLoadDic valueForKeyPath:@"data.states.title"];
